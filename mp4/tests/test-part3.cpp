...
TEST_CASE("server - loads `/microbe.html`", "[weight=10][part=3]") {
  int port = launch_server();

  char curl[1000];
  sprintf(curl, "curl --max-time 15 -f http://localhost:%d/microbe.html >TEST_out.txt", port);

  CAPTURE(curl);
  REQUIRE( system(curl) == 0 );
  REQUIRE( system("diff TEST_out.txt static/microbe.html") == 0 );
  system("rm TEST_out.txt");
