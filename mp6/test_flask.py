...
def test_not_valid_png(test_client):
    ...
    data = {'png': (io.BytesIO(test_file.read()), 'luther.gif')}
    response = test_client.post('/extract', data=data, content_type='multipart/form-data')
    assert(response.status_code == 422)

def test_hidden_gif_luther(test_client):
    test_file = open('sample/luther.png', 'rb')

    data = {'png': (io.BytesIO(test_file.read()), 'luther.png')}
    response = test_client.post('/extract', data=data, content_type='multipart/form-data')
    assert(response.status_code == 200)
    assert(response.get_data() == open('sample/luther.gif','rb').read())
...
def test_get_gif_luther(test_client):
    test_hidden_gif_luther(test_client)
